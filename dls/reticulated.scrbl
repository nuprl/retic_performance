...
Reticulated Python is implemented as a source-to-source translator, which means that it takes a Python program and generates Python code after adding the necessary checks to typecheck the program. The code is then executed as regular Python. It has three modes of implementation: Monotonic, Guarded and Transient, each with a different way of providing soundness guarantees. Our analysis uses Transient mode.
