...
Sound gradual typing allows us to safely combine typed and untyped code, but to accomplish soundness, gradual typing adds extra checks during runtime. There are currently a few implementations of sound gradual typing, but none of them seem to be efficient. Studies by  Takikawa et al on Typed Racket , a macro-gradual type system show a significant overhead of Typed Racket compared Racket. The method used in this study explores the partial conversions from typed to untyped code by measuring the performance of 2^n configurations where n is the number of modules in a given benchmark. It also employs the linear sampling approach by generating a linear number of samples of random typed/untyped ratios over modules. It then uses a lattice oriented approach to interpret the results. 
