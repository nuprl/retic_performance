...
In this paper, we consider Reticulated Python, a micro-gradual type system for Python. We port 8 benchmarks to Python, and measure the performance in the two ways. Consider a program with n functions. We take 2^n configurations obtained by choosing to annotate or to not annotate every function in the program. This method allows us to capture as many varied configurations over types as possible in 2^n configurations. Secondly we select n random configurations. This method annotates each atomic expression in the program randomly. We interpret the results according to the lattice-oriented approach from Takikawa et al.

In summary, we make two main contributions: We test Siek's hypothesis by conducting a proper performance evaluation on a micro-gradual typing system, and validate the linear sampling method described by Takikawa et al.
