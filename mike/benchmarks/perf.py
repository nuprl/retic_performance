...
def MeasureChameleonV2(python, options):
    bm_path = Relative("performance/bm_chameleon_v2.py", python, options)
    lib_path = Relative("lib/Chameleon-2.22/src", python, options)
    bm_env = {"PYTHONPATH": lib_path}
    return MeasureGeneric(python, options, bm_path, bm_env)

@VersionRange('2.7', None)
def BM_Chameleon_V2(*args, **kwargs):
    return SimpleBenchmark(MeasureChameleonV2, *args, **kwargs)
...
@VersionRange(None, '3.5')
def BM_Django_v2(*args, **kwargs):
    return SimpleBenchmark(MeasureDjangoV2, *args, **kwargs)


def MeasureDjangoV3(python, options):
    bm_path = Relative("performance/bm_django_v3.py", python, options)
    django_path = Relative("lib/Django-1.9", python, options)
    bm_env = {"PYTHONPATH": django_path}
    return MeasureGeneric(python, options, bm_path, bm_env)

@VersionRange('2.7', None)
def BM_Django_v3(*args, **kwargs):
    return SimpleBenchmark(MeasureDjangoV3, *args, **kwargs)
...
# automatically.
BENCH_GROUPS = {"default": ["2to3", "chameleon_v2", "django_v3", "nbody",
                ...
                "apps": ["2to3", "chameleon_v2", "html5lib", "rietveld",
                         "spambayes", "tornado_http"],
                "calls": ["call_simple", "call_method", "call_method_slots",
                          "call_method_unknown"],
                "math": ["float", "nbody", "pidigits"],
                "template" : ["slowspitfire", "django_v3", "mako_v2"],
                "logging": ["silent_logging", "simple_logging",
                            "formatted_logging"],
                # These are removed from the "all" group
                "deprecated": ["chameleon", "django", "django_v2",
                               "iterative_count", "json_dump", "mako",
                               "threaded_count", "spitfire"],
                }
