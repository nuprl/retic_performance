...
def to_xy(pos:int)->(int, int):
    y, x = divmod(pos, SIZE)
    return x, y

@fields({'pos':int, 'zobrist_strings':List(int), 'color':int, 
         'board':Board, 'timestamp':int, 'removestamp':int, 'reference':Square,
         'ledges':int,
         'used':bool, 'neighbours':List(Square)})
class Square:
    ...
    def remove(self, reference:Square, update=True):
...
#                self.board.white_dead += 1
    ...
    def find(self, update=False)->Square: 
    ...
    def __repr__(self:Dyn):
        return repr(to_xy(self.pos))

@fields({'empties':List(int), 'board':{'useful':Function(NamedParameters([('pos',int)]), int)}, 'empty_pos':List(int)})
class EmptySet:
    def __init__(self, board:{'useful':Function(NamedParameters([('pos',int)]), int)}):
        self.board = board
    ...
    def add(self:EmptySet, pos:int):
        self.empty_pos[pos] = len(self.empties)
        self.empties.append(pos)

    def remove(self:EmptySet, pos:int):
        self.set(self.empty_pos[pos], self.empties[len(self.empties)-1])
        self.empties.pop()

    def set(self:EmptySet, i:int, pos:int): 
        self.empties[i] = pos
        self.empty_pos[pos] = i

@fields({'hash':int})
class ZobristHash:
    def __init__(self, board:{'squares':List(Square)}):
        self.hash_set = set()
        self.hash = 0
    ...
    def add(self:ZobristHash):
        self.hash_set.add(self.hash)
    
    def dupe(self:ZobristHash)->bool:
        return self.hash in self.hash_set

@fields({'squares':List(Square), 'emptyset':EmptySet, 'zobrist':ZobristHash,
         'color':int, 'finished':Bool, 'history':List(int),
         'white_dead':int, 'black_dead':int, 'lastmove':int})
class Board:
    def __init__(self):
        self.squares = []
...
class UCTNode:
    ...
    def play(self:UCTNode, board:Board):
        """ uct tree search """
        color = board.color
        node = self
        path = [node]
