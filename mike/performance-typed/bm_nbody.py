...
def advance(dt:float, n:int, bodies=SYSTEM, pairs=PAIRS):
    for i in xrange(n):
        for (([x1, y1, z1], v1, m1),
...
def offset_momentum(ref:(float, float, float), bodies=SYSTEM, px=0.0, py=0.0, pz=0.0):
    for (r, [vx, vy, vz], m) in bodies:
        px -= vx * m
        py -= vy * m
        pz -= vz * m
...
def test_nbody(iterations:int, timer):
    # Warm-up runs.
    report_energy()
    advance(0.01, 20000)
    report_energy()
