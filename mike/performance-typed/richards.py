...
@fields({'ident':int, 'kind':int, 'datum':int, 'data':List(int)})
class Packet(object):
    def __init__(self,l,i:int,k:int):
        self.link = l
        self.ident = i
        self.kind = k
        self.datum = 0
...
@fields({'control':int, 'count':int})
class IdleTaskRec(TaskRec):
    def __init__(self):
        self.control = 1
        self.count = 10000

@fields({'work_in':Dyn, 'device_in':Dyn})
...
@fields({'destination':int, 'count':int})
...
@fields({'packet_pending':bool, 'task_waiting':bool, 'task_holding':bool})
...
TASKTABSIZE = 10
@fields({'taskTab':List(Dyn), 'holdCount':int, 'qpktCount':int})
...
@fields({'ident':int, 'priority':int, 'packet_pending':bool, 'handle':TaskRec}) 
class Task(TaskState):


    def __init__(self,i:int,p:int,w,initialState:TaskState,r:TaskRec):
    ...
    def fn(self,pkt:Packet,r:TaskRec)->Task:
        raise NotImplementedError


    def addPacket(self,p:Packet,old:TaskState)->TaskState:
    ...
    def hold(self)->Packet:
        taskWorkArea.holdCount += 1
        self.task_holding = True
        return self.link


    def release(self,i:int)->TaskState:
        t = self.findtcb(i)
    ...
    def qpkt(self,pkt:Packet)->TaskState:
...
class DeviceTask(Task):
    def __init__(self,i:int,p:int,w,s:TaskState,r:TaskRec):
...
def entry_point(iterations:int)->(bool, float, float):
    r = Richards()
    startTime = time.time()
